...
// Esta función realiza comprobación de si es un nuevo registro o no para ocultar el botón de eliminar.
function comprobar(){
...
/*La función realiza la acción de vaciar los elementos del formulario*/
function clearForm(){

	var formulario = document.getElementById('formContacto');
	var combo = document.getElementsByName('categoria');

	// Se establecen las cajas a 0;
	...
	formulario.telefono1.value = "";
	formulario.telefono2.value = "";
	formulario.empleado.checked = 0;
}

// Habilita los cuadros del formulario.
function enableForm(){

	// Se realizan los guardados de los elementos que se utilizarán.
	var aside = document.getElementById('detalles');
	var formulario = document.getElementById('formContacto');	
	var combo = document.getElementsByTagName('categoria');

	// Se habilitan los botones del formulario.
	formulario.nombre.removeAttribute("readonly", false);
	formulario.fechaNacimiento.removeAttribute("readonly", false);
	formulario.dni.removeAttribute("readonly", false);
	formulario.provincia.removeAttribute("readonly", false);
	formulario.localidad.removeAttribute("readonly", false);
	formulario.direccion.removeAttribute("readonly", false);
	formulario.codpostal.removeAttribute("readonly", false);
	formulario.telefono1.removeAttribute("readonly", false);
	formulario.telefono2.removeAttribute("readonly", false);
	formulario.empleado.disable = false;
	//combo.removeAttribute("readonly", false);

	aside.style.display = 'block';
}

// Deshabilita los cuadros del formulario.
function disableForm(){
	...
	formulario.fechaNacimiento.readOnly = true;
	formulario.dni.readOnly = true;
	formulario.provincia.readOnly = true;
	formulario.localidad.readOnly = true;
	formulario.direccion.readOnly = true;
	formulario.codpostal.readOnly = true;
	formulario.telefono1.readOnly = true;
	formulario.telefono2.readOnly = true;
	formulario.empleado.disable = true;
	combo.disable = true;
}

// Realiza los cambios en el formulario tras el guardado de datos.
function save(){

	var caja = document.getElementById('detalles');
	// Se muestran/ocultan los objetos que se van a utilizar.
...
// Realiza los cambios en el formulario para la edición de los datos.
function edit(){
...
window.onload = function(){/*hace que se cargue la función lo que predetermina que div estará oculto hasta llamar a la función nuevamente*/
	// Se muestra el aside y se bloquea.
	var aside = document.getElementById('detalles');
	comprobar();
	disableForm();
	aside.style.display = 'block';
